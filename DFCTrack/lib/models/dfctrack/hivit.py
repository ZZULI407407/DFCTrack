...
from lib.models.dfctrack.hivit_backbone import BaseBackbone
...
class Attention(nn.Module):
    ...
    def forward_one(self, z, x, return_attention=False):
        B, _, C = z.shape
...
class BlockWithRPE(nn.Module):
    ...
    def forward(self, x, rpe_index=None, mask=None, return_attention=False):
    ...
    def forward_one(self, z, x, return_attention=False):
        if return_attention:
            feat, attn = self.attn.forward_one(self.norm1(z), self.norm1(x), return_attention=return_attention)
            x = x + self.drop_path(feat)
            x = x + self.drop_path(self.mlp(self.norm2(x)))
            return x, attn
        else:
...
class HiViT(BaseBackbone):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000,
                 ...
                 norm_layer=nn.LayerNorm, ape=True, rpe=True, patch_norm=True, use_checkpoint=False, dropout=0.1,
                 **kwargs, ):
        super().__init__()
...
def _create_hivision_transformer(pretrained=False, default_cfg=None, **kwargs):
    ...
    if pretrained:
            ...
            # else:
            #     # 使用 torch.load 加载预训练权重文件（通常是 .pth 或 .pt 格式），并将权重加载到 CPU 上
            #     checkpoint = torch.load(pretrained, map_location="cpu")
            #     # 获取 checkpoint['model'] 中的所有键名，这些键名对应模型的权重参数
            #     keys = checkpoint['model'].keys()
            #     for k in list(keys):
            #         # 如果键名中包含 'blocks.'，则表示该键名对应 Transformer 中的某个块（block）的权重
            #         if 'blocks.' in k:
            #             # 如果键名中包含 'qkv.weight'，表示该键名对应多头自注意力机制中的 qkv 权重
            #             if 'qkv.weight' in k:
            #                 # 将 qkv 权重从原始形状重新调整为 (3, 768, 768)，其中 3 表示 q、k 和 v，768 是维度大小
            #                 qkv = checkpoint['model'][k].reshape(3, 768, 768)
            #                 # key = k.replace('blocks', 'cross_blocks')
            #                 # 将原始键名中的 'qkv.weight' 替换为 'q.weight'、'k.weight' 和 'v.weight'，分别对应 q、k 和 v 的权重键名
            #                 q_key = k.replace('qkv.weight', 'q.weight')
            #                 k_key = k.replace('qkv.weight', 'k.weight')
            #                 v_key = k.replace('qkv.weight', 'v.weight')
            #                 # 将 qkv 权重的第 0、1、2 个部分分别赋值给 q、k 和 v 的权重键名对应的值
            ...
            #                 # key = k.replace('blocks', 'cross_blocks')
            #                 # 将原始键名中的 'qkv.bias' 替换为 'q.bias'、'k.bias' 和 'v.bias'，分别对应 q、k 和 v 的偏置键名
            #                 q_key = k.replace('qkv.bias', 'q.bias')
            #                 k_key = k.replace('qkv.bias', 'k.bias')
            #                 v_key = k.replace('qkv.bias', 'v.bias')
            #                 # 将 qkv 偏置的第 0、1、2 个部分分别赋值给 q、k 和 v 的偏置键名对应的值
            #                 checkpoint["model"][q_key] = qkv[0]
            #                 checkpoint["model"][k_key] = qkv[1]
            #                 checkpoint["model"][v_key] = qkv[2]
            #     # 使用 model.load_state_dict 方法将处理后的权重加载到模型中
